...
def run_training():
    ...
    if ARGS.data_set in ['mnist', 'fashion_mnist']:
        ...
        '''
        df = pd.read_csv(f"train.tsv", sep='\t', header = None)
        data1 = df.values[:,1:]
        df = pd.read_csv(f"test.tsv", sep='\t', header = None)
        data2 = df.values[:,1:]

        from sklearn.model_selection import train_test_split
        train_x,valid_x,train_labels,valid_labels = train_test_split(data1[:,1:], data1[:,0], test_size=0.2, random_state=13)
